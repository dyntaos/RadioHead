...
// Changing these types will be sufficient to alter the size of addresses (to and from) but
// will obviously be only able to communicate with RadioHead libraries of the same type sizes

typedef uint8_t  rh_id_t;
...
// Changing size of rh_flags_t may cause issues with RH_FLAGS_RESERVED (RHGenericDriver.h)
// TODO: This needs investigation
typedef uint8_t  rh_flags_t;
// Cannot use sizeof(), as this value is used in an #if preprocessor statement below
...
#define ntoh_id(x) ntohs(x)
#define hton_id(x) htons(x)

#elif RH_ID_SIZE == 4

#define ntoh_id(x) ntohl(x)
...
#define ntoh_flags(x) (x)
#define hton_flags(x) (x)

#elif RH_FLAGS_SIZE == 2

#define ntoh_flags(x) ntohs(x)
#define hton_flags(x) htons(x)

#elif RH_FLAGS_SIZE == 4

#define ntoh_flags(x) ntohl(x)
